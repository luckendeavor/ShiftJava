...
##### 2. Xml映射文件中，除了常见的 select|insert|updae|delete 标签之外，还有哪些标签？
...
##### 7. Mybatis 动态 SQL 是做什么的？都有哪些动态 SQL？能简述一下动态 SQL的执行原理不？

答：Mybatis 动态 sql 可以让我们**在 Xml 映射文件内**，以**标签的形式编写动态 sql**，完成**逻辑判断和动态拼接 sql 的功能**，Mybatis 提供了 9 种**动态 sql 标签** `trim|where|set|foreach|if|choose|when|otherwise|bind`。

其执行原理为，使用 **OGNL** 从 sql 参数对象中**计算表达式的值**，根据**表达式的值动态拼接 sql**，以此来完成动态 sql 的功能。
